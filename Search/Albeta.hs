...
pvSearch nst !a !b !d = do
    ...
    if useTTinPv && hdeep >= d && (
       ...
       then  do
           ...
           let nst' = if hdeep > 0 && e' /= Move 0 && (tp /= 0 || nullSeq (pvcont nst))
           ...
           if movno nstf == 0
              then do	-- no legal move - it looks like statusVal does not work correctly!
              ...
              else do	-- here we had legal moves
                  let s = cursc nstf
                  pindent $ "<= " ++ show s
                         ...
                         when (de >= minToStore) $ do
                              ...
                              lift $ do
                                  let es = unseq $ pathMoves s
                                      mv = if null es then Move 0 else head es
                                  {--
                                  mv <- if null es
                                           then do
                                               -- logmes $ "Failed low (P) with empty path: " ++ show s
                                               mr <- getNextMove gedges
                                               case mr of
                                                   Nothing     -> return $ Move 0 -- this shouldn't happen
                                                   Just (m, _) -> return m
                                           else return (head es)
                                  --}
       ...
       then  do
           ...
           if abrt || nmhigh
              then do
                ...
                gedges <- genAndSort nst' bGrain b d
                !nodes0 <- gets (sNodes . stats)
                -- futility pruning?
                ...
                if movno nstf == 0
                   then do	-- no legal move - staticVal seem not to work
                       chk <- lift tacticalPos
                       let s = if chk then matedPath else staleMate
                       viztreeScore $ "noMove: " ++ show (pathScore s)
                       pindent $ "<= " ++ show s
                       ...
                       pindent $ "<: " ++ show s
                       let !de = max d $ pathDepth s
                       when (de >= minToStore && s < b) $ do	-- we failed low, store it in TT
                           !nodes1 <- gets (sNodes . stats)
                           lift $ do
                               let es = unseq $ pathMoves s
                                   mv = if null es then Move 0 else head es
                               ...
                               let typ = 0
                                   !deltan = nodes1 - nodes0
                               ttStore de typ (pathScore b) mv deltan
                       if s > bGrain
                          then return $! bestPath s b
                          else return $! bestPath s bGrain
    where bGrain = b -: scoreGrain
...
pvSLoop :: Path -> Int -> Bool -> NodeState -> MGen -> Search NodeState
pvSLoop b d p = go
    where go !s g = do
...
pvZLoop :: Path -> Int -> Bool -> Bool -> NodeState -> MGen -> Search NodeState
...
-- and the killers
genAndSort :: NodeState -> Path -> Path -> Int -> Search MGen
genAndSort nst a b d = do
    ...
    adp <- gets absdp
    (gp, kl)  <- lift $ do
         gp <- genMoves d adp True
         kl <- filterM isMoveLegal $ killerToList (killer nst)
         return (gp, kl)
    return $ bestFirst path kl gp
...
-- {-# INLINE pvLoop #-}
pvLoop :: (s -> Move -> Search (Bool, s)) -> s -> MGen -> Search s
pvLoop f = go
    where go !s g = do
              mr <- lift $ getNextMove g
              case mr of
...
pvQSearch !a !b = do
    ...
    if tact
       then do
           ...
           (n, s) <- pvQLoop b (0, a) g
           -- if no move: I'm mated
           if n == 0 then return (-mateScore) else return s
               ...
               then do
                   ...
                   return b
                       ...
                       then do
                           ...
                           g <- lift genTactMoves
                           (n, s) <- if stp > a
                              then pvQLoop b (0, stp) g
                              else pvQLoop b (0, a)   g
                           return s	-- should we try other moves here too?

pvQLoop :: Int -> (Int, Int) -> MGen -> Search (Int, Int)
pvQLoop b = go
    where go (n, s) g = do
              mr <- lift $ getNextMove g
              case mr of
                  Nothing      -> return (n, s)
                  Just (e, g') -> do
                      (!cut, (!n', !s')) <- pvQInnerLoop b (n, s) e
                      if cut then return (n', s')
                             else go (n', s') g'

pvQInnerLoop :: Int -> (Int, Int) -> Move -> Search (Bool, (Int, Int))
pvQInnerLoop !b (!n, !a) e = do
    -- qindent $ "-> " ++ show e
    ...
    if legalResult r
       then do
           ...
           if sc >= b
              then return (True, (n+1, b))
              else do
                  !abrt' <- gets abort
                  if sc > a
                     then return (abrt', (n+1, sc))
                     else return (abrt', (n+1, a))
...
bestFirst :: [Move] -> [Move] -> (MGen, MGen) -> MGen
bestFirst path kl (g1, g2)
    | null path = makeListGen [ g1, makePureGen kl, makeFiltGen kl g2 ]
    | otherwise = makeListGen [ makePureGen es, makeFiltGen es g1, makePureGen kl, makeFiltGen ek g2 ]
    where (e:_) = path
          es    = [e]
          ek    = e : kl
